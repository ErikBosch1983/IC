...
if ($poulenr != "false")
	{
		$onbekenden = $db->get_array("SELECT * FROM IC_pouleindeling WHERE poulenr='" . $poulenr . "' AND plaats='100' ORDER BY volgorde");
		
		// controleer op het te spelen wedstrijden in de extra ronde
		$matches = $db->num_rows("SELECT * FROM IC_matches WHERE size='-" . count($onbekenden) . "'");
		$wedstrijden = $db->num_rows("SELECT * FROM IC_wedstrijden WHERE poulenr='" . $poulenr . "' AND volgorde<'0'");
		
		if ($wedstrijden == $matches)
			{
				echo "De aantallen komen overeen";
			}
