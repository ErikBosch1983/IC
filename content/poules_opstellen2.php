...
if ($competitienr != "false" && $gewichtsklassenr != "false")
	{
		$knip = (int)$_GET["knip"];
		if ($knip != 0)
			{
		...
		if ($knip == 0)
			{
				...
				if ($aantal == 0)
					...
					{	
						...
						// $aantalwit = bepaal_aantal_van_graduatie ($competitienr, $gewichtsklassenr, "6", "inclusief", $poulenr, 0, 0);
						// $aantalwit = bepaal_aantal_van_graduatie ($competitienr, $gewichtsklassenr, "6", "exclusief", $poulenr, 0, 0);
						// $aantalwit = bepaal_aantal_van_graduatie ($competitienr, $gewichtsklassenr, "6", "inpoule", $poulenr, 0, 0);
								...
								foreach ($pouleinaanmaak as $poulelid)
									{
										...
										if ($poulelid["graduatie"] == 6){$gradwit = $gradwit + 1;}
										if ($poulelid["graduatie"] == 5){$gradgeel = $gradgeel + 1;}
										if ($poulelid["graduatie"] == 4){$gradoranje = $gradoranje + 1;}
								...
								foreach ($clubs as $club)
									{
										...
										if ($aantalclub > 0)
											{
												echo $club["clubnaam"] . " heeft " . $aantalclub . " deelnemers, dat is " . round ($aantalclub/$poules) . " per poule en al " . $deelnemerss[$club["clubnr"]] . " in de poule<br>";
					...
					{
						echo "Geen judoka's in deze gewichtsklasse ingeschreven";
					}
			}
	}
